...
router.post("/", async (req, res) => {
  try {
    ...
    const insertPolygon = await pool.query("INSERT INTO polygon (nama_polygon, geom, deskripsi) VALUES($1, ST_SetSRID(ST_GeomFromText($2), 4326), $3) RETURNING *", [nama_polygon, geom, deskripsi])
...
router.patch("/:id", async (req, res) => {
  try {
    ...
    const updatePolygon = await pool.query("UPDATE polygon SET nama_polygon = $2, geom = ST_SetSRID(ST_GeomFromText($3), 4326), deskripsi = $4 WHERE id_polygon = $1", [id, nama_polygon, geom, deskripsi])
